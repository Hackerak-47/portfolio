...
function ScrollDots({ total, current }) {
  return (
    <div style={{ position:"fixed", right:28, top:"50%", transform:"translateY(-50%)", zIndex:200, display:"flex", flexDirection:"column", gap:10 }}>
      {Array.from({ length: total }).map((_, i) => (
        <div key={i} style={{ width: i === current ? 8 : 5, height: i === current ? 8 : 5, borderRadius:"50%", background: i === current ? "#ff3cac" : "rgba(255,255,255,0.2)", transition:"all 0.3s" }} />
      ))}
    </div>
  );
}
...
function SkillsSection() {
  ...
  const skills = [
    ...
    { name:"Supabase", cat:"fw" }, { name:"Firebase", cat:"fw" }, { name:"MongoDB", cat:"fw" },
    { name:"SQL", cat:"lang" }, { name:"Gemini API", cat:"lib" }, { name:"Tailwind CSS", cat:"lib" },
    { name:"Python", cat:"lang" }, { name:"Java", cat:"lang" }, { name:"Flask", cat:"fw" },
    { name:"Framer Motion", cat:"lib" }, { name:"Recharts", cat:"lib" }, { name:"Mongoose", cat:"lib" },
    { name:"C", cat:"lang" }, { name:"Git", cat:"tool" }, { name:"Docker", cat:"tool" },
  ];
...
function SkillPill({ name, color, delay }) {
  ...
  return (
    <div
      ...
      style={{
        padding:"14px 18px",
        border: hov ? `1px solid ${color}66` : "1px solid rgba(255,255,255,0.06)",
        borderRadius:8,
        fontFamily:"'Space Mono',monospace",
        fontSize:12,
...
function ProjectsSection() {
  const projects = [
    ...
    {
      num:"03", period:"Dec 2025 – Jan 2026",
      name:"Focus Forge",
      ...
      live:"https://focus-forge-five-bay.vercel.app/",
    },
  ];
  ...
  return (
    ...
    >
      ...
      <div className="project-card-left">
        <div style={{ fontFamily:"'Space Mono',monospace", fontSize:10, letterSpacing:"2px", color:"rgba(255,255,255,0.2)" }}>
          {p.num} · {p.period}
        </div>
      ...
      <div style={{ display:"flex", gap:36 }}>
        {sections.map((s, i) => (
          <button key={s} onClick={() => document.getElementById(ids[i]).scrollIntoView({ behavior:"smooth" })}
            style={{ fontFamily:"'Space Mono',monospace", fontSize:11, letterSpacing:"2px", textTransform:"uppercase", color: active === i ? "#fff" : "rgba(255,255,255,0.35)", background:"none", border:"none", cursor:"pointer", position:"relative", padding:"4px 0", transition:"color 0.3s" }}>
            {s}
            <span style={{ position:"absolute", bottom:-4, left:0, width: active === i ? "100%" : 0, height:1, background:"#ff3cac", transition:"width 0.3s" }} />
          </button>
        ))}
      </div>
    </nav>
    ...
    <>
      ...
      <style>{`
        *{margin:0;padding:0;box-sizing:border-box;cursor:none!important;}
        ...
        ::-webkit-scrollbar-thumb{background:#ff3cac44;border-radius:3px;}
        .projects-grid{display:grid;grid-template-columns:1fr;gap:24px;margin-top:32px;width:100%;}
        .project-card{background:rgba(255,255,255,0.03);border:1px solid rgba(255,255,255,0.06);border-radius:10px;padding:32px 40px;position:relative;overflow:hidden;transition:all 0.4s;display:flex;flex-direction:row;gap:40px;align-items:center;text-align:left;}
        .project-card:hover{border:1px solid rgba(255,60,172,0.3);transform:translateY(-4px);box-shadow:0 20px 50px rgba(0,0,0,0.4);}
